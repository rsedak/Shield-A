/* D0-D7, RV1
 *  turning on LEDs from the centre to the edge
...
void loop() {
  ...
  if ( lastActiveLED > activeLED ) {
    digitalWrite(4 + lastActiveLED, LOW);
    digitalWrite(3 - lastActiveLED, LOW);
  } else {
    // turn the active LEDs on
    digitalWrite(4 + activeLED, HIGH);
    digitalWrite(3 - activeLED, HIGH);
  }
