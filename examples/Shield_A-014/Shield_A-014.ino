...
void loop() {
 ...
 for ( int i = 0; i <= 7; i++) {
    // turn first and even LEDs in raw on
    for ( int j = 0; j <= 1; j++) {
      led = i + j * 2;
      // if LED is "over" the left edge, 
      // turn on first LED on right edge
      if ( led > 7 ) {
        led = led - 8;
      }
   ...
   // turn first and even LEDs in raw off
   for ( int j = 0; j <= 1; j++) {
     led = i + j * 2;
     // if LED is "over" the left edge, 
     // turn off first LED on right edge
     if ( led > 7 ) {
        led = led - 8;
      }
