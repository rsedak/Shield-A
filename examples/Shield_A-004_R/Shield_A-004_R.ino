/* D0-D7 
 * turning on LEDs  - from D0 to D7; 
 * turning off LEDs - from D7 to D0
...
void loop() {
  ...
  // turn the LEDs D0 - D7 on, one by one with 200ms delay
  for ( int i = 0; i <= 7; i++) {
    PORTD |= (1 << i);
    delay(200);
  }

  // turn the LEDs D7 - D0 off, one by one with 200ms delay
  for ( int i = 7; i >= 0; i--) {
    PORTD &= ~(1 << i);
    delay(200);
  }
